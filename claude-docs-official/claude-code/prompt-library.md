...
export const PromptLibrary = ({text = {}, labels = {}, tagLabels = {}, phaseLabels = {}, sourceLabels = {}, catLabels = {}}) => {
  ...
  const linkify = s => {
    ...
    for (let m; m = re.exec(s); ) {
      ...
      out.push(<a key={m.index} href={doc(m[2])}>{m[1]});
      last = re.lastIndex;
    }
  ...
  const promptBody = p => {
    ...
    return <code></code>
      ...
      if (!m) return <span key={idx}>;      const k = m[1];      const val = fillOf(p, k);      return <input key={idx} type="text" className="pl-slot" value={val} placeholder={p.slots[k] || k} aria-label={k} style={{        width: widthFor(val || p.slots[k])      }} onChange={e => setFill(p.id, k, e.target.value)} onFocus={e => e.target.select()} onClick={e => e.stopPropagation()} />;    })}      {part}</code>

 : <code className="pl-prompt-preview">{preview(p)}</code>}
        {open && 
