...
- **Workflow engine design v2**: archived to `v3-final/` — `workflow_v2_plan-CN_archive.md`, `project_flow_plan-CN_archive.md`, `workflow_v2_risks-CN_archive.md` (originals renamed `notes/*_moved.md`). Fixed topology and human-gated experience-layer self-iteration. The v1 self-evolving DSL's 127-paper risk review is at `v3-final/V1-workflow_risk_review-CN_archive.md`; remaining DSL files stay under `project/to-do-future/DSL/`. Not the active plan.
- **v3-final/ design archive**: canonical V1→V2→V3 design/audit docs consolidated in `v3-final/` (index `v3-final/README.md`); filename tag convention `_latest` / `_archive` / `_deprecated` / `_moved` / `_V1_finished`. Originals left in place with `_moved` breadcrumbs. `WORK_LOG.md` stays living at repo root (not moved).
